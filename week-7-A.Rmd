...
q2 <- q_resamples %>%
         mutate(hours_sampled = map(boot, ~.$hours),
                # hours_sampled = map(boot, ~pull(., hours))
         mean_hours = map_dbl(hours_sampled, ~mean(.)))
...
```{r sc3}
ggplot(q2, aes(x = mean_hours, y = after_stat(count/sum(count)))) + 
  geom_histogram(binwidth = 0.005, fill = 'chartreuse') +
  geom_density() + 
  labs(title = "Posterior Distribution for the Mean Hours in College Courses",
       x = 'Mean Hours',
       y = 'Probability')
